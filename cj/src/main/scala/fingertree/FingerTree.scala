...
object FingerTree {
  ...
  def append3[V, A](l: FingerTree[V, A], m: List[A], r: FingerTree[V, A])(implicit M: Measured[V, A]): FingerTree[V, A] = {
    ...
    import Syntax._
    type DV[+A] = Digit[V, A]
    implicit val DConsable: Consable[List[A], FingerTree[V, A]] = Consable(ReduceList.reduceR(((a, b) => a +: b ): (A, => FingerTree[V, A]) => FingerTree[V, A]))
    implicit val DSconable: Sconable[FingerTree[V, A], List[A]] = Sconable(ReduceList.reduceL(((a, b) => a :+ b ): (FingerTree[V, A], A)    => FingerTree[V, A]))
    (l, m, r) match {
